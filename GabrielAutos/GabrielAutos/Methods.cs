...
namespace Autos
{
	public class Avto
	{
        ...
        public int topCheck; //проверка корректности заправки
        public int nomCheck; //проверка совпадения номера
        public int acc; //количество возможных аварий
		...
		private void Move(double x1, double y1, double x2, double y2)
		{
			...
			while (this.km > 0)
			{
				...
				else
				{
					...
					Console.WriteLine("Вы проехали " + Math.Round((double)this.top / this.ras * 100, 2) + " км. Для дальнейшей поездки необходимо дозаправить " + Ostatok() + " л бензина. Поедете дальше?");
